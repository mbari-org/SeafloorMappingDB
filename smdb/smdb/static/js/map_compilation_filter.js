...
let feature = L.geoJSON(missions, {
  ...
  .bindPopup(function (layer) {
    ...
    return (
      "<a target='_blank' rel='noopener noreferrer' href='" + window.location.origin + '/missions/' + _escapeHtml(slug) + "/'>" +
  ...
  for (var i = 0; i < missions.features.length; i++) {
    ...
    var marker = L.marker(anchor, {
      icon: L.divIcon({
        ...
        html: "<a target='_blank' rel='noopener noreferrer' href='" + window.location.origin + '/missions/' + _escapeHtml(slug) + "/'>" + _escapeHtml(slug) + "</a>",
      }),
    });
...
const FilterControl = L.Control.extend({
  onAdd: function (map) {
    ...
    container.setAttribute("aria-label", "Filter Compilations");
    const filterTooltip = L.DomUtil.create("span", "map-control-tooltip", buttonWrapper);
    filterTooltip.textContent = "Filter Compilations";
    ...
    title.textContent = "Filter Compilations";
    title.style.cssText =
    ...
    const copyForm = function () {
      ...
      const formWrapper = formContainer.querySelector("#filter-form-compilation");
      ...
      clonedForm.id = "filter-form-compilation";
      clonedForm.className = "filter-form";
      clonedForm.setAttribute("data-filter-type", "compilation");
      ...
      filterTypeInput.value = "compilation";
      ...
      filterBtn.id = "compilationFilterSubmit";
      ...
      clearBtn.type = "reset";
      clearBtn.id = "compilationFilterCancel";
      ...
      body.querySelectorAll(".row, .compilationRow").forEach(function (row) {
...
var DrawSquareButton = L.Control.extend({
  onAdd: function (map) {
    ...
    btn.setAttribute("aria-label", "Draw a square around compilations to filter by area.");
    var tooltipEl = L.DomUtil.create("span", "map-control-tooltip", wrapper);
    tooltipEl.textContent = "Draw a square to filter compilations by area";
